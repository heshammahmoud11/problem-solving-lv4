...
bool isDate_within_Period(stPeriod period, stDate checkDate)
{
    if(isDate1_after_Date2(checkDate, period.startDate) && (isDate1_before_Date2(checkDate, period.endDate)))
    {
        return true;
    }else 
        return false;
...
int main()
{
    ...
    if(isDate_within_Period(period, checkDate))
